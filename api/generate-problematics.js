...
export default async function handler(req, res) {
  ...
  if (isOwnerRequest(body)) {
    try { requireOwner(req); } catch (error) {
      return res.status(Number(error?.status) || 401).json({ error: error?.message || "Session administrateur invalide." });
    }
  }
